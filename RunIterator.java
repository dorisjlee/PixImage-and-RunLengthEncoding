...
public class RunIterator implements Iterator {
  ...
  public int[] next() {
    ...
    //Check if there are still more runs in the iterator
      if (this.hasNext() != true){ 
        int [] dummy = new int[3];
        return dummy;
      }
      // Replace the following line with your solution.
      //1)pixel_count 2) r value 3)g value 4)b value
      if (start.next != null && start.item != null){
        int[] array = new int[] { start.item.length, start.item.red, start.item.green,start.item.blue };
        start=start.next;
        return array;
      }
      return new int[]{} ;
